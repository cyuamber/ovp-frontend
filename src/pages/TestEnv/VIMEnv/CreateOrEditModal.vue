        ...
        <div v-else-if="currentTab === 'VNFM ENV'">
          ...
          >
            ...
            <a-input-password
              v-if="item.title === 'Password'"
              type="password"
              v-decorator="[item.key, { initialValue: initValues[item.key] }]"
            ></a-input-password>
          </a-form-item>
        </div>
...
export default {
      ...
      if (val.length) {
        if (!this.isEdit) {
          this.initCloudType = {
            ...
            code:
              this.currentTab === "MANO ENV" ? val[0].dictValue : val[0].code
          };
          if (this.initCloudType.name.toUpperCase() === "ONAP") {
            this.onapProfilesShow = true;
          } else {
  ...
  methods: {
    ...
    handleDeleteONAPProfile(record) {
      ...
      this.onapProfileDataSource = onapProfileDataSource.filter(
        item => item.customParamsNumber !== record.customParamsNumber
      );
    },

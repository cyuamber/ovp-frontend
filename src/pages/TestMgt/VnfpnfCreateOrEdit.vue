  ...
  <a-modal v-bind:title="title" v-model="showModal" :footer="null" @cancel="handleCancel">
    <template>
      <a-form :form="form" @submit="handleSubmit">
        ...
        >
          <a-input
            v-decorator="['XNFName',{ rules: [{ required: true,message: currentTab +' Name is required' }],initialValue:SuiteSingleData.tesyMeterName }]"
          />
        </a-form-item>
        <a-form-item
        ...
        >
          <a-input
            v-decorator="['XNFVendor',{ rules: [{ required: true, message: currentTab +' Vendor is required' }],initialValue:SuiteSingleData.tesyMeterVendor }]"
          />
        </a-form-item>
        <a-form-item
          :label="currentTab+' Version'"
          :label-col="{ span: 7 }"
          :wrapper-col="{ span: 12 }"
        >
          <a-input
            v-decorator="['Version',{ rules: [{ required: true, message: currentTab +' Version is required' }],initialValue:SuiteSingleData.tesyMeterVersion }]"
          />
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      showModal: true,
      title: this.isEdit
        ? "Edit " + this.currentTab + " TT"
        : "Create " + this.currentTab + "TT",
      spin: false,
      count: 0,
      disabled: false
    };

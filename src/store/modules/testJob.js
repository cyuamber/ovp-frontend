...
const actions = {
            ...
            .then((res) => {
                ...
                if (res.code === 200) {
                    ...
                    let tableData = res.body.map((item, index) => {
                        ...
                        item.index =
                            res.body.length * (state.pagination.current - 1) +
                            index +
                            1;
                        item.actions = [
    ...
    getTestFail({ dispatch }, data) {
        axiosget(
            API.testJobMgt.testFailedDetail.replace(
                ":requestId",
                data.requestId
            )
        )
    ...
    runTestJobMGT({ dispatch, commit }, { data }) {
        axiosput(
            API.testJobMgt.testJobStart.replace(":jobId", data.jobId)
        ).then(
            (res) => {
                if (res.code === 200) {
                    dispatch(
                        ...
                        {
                            type: "success",
                            toast: "Successfully started testing",
                        },
                        { root: true }
                    );
                    data.status = "RUNNING";
            ...
            (res) => {
                if (res.code === 200) {
                    ...
                    if (
                        ...
                        res.body.scheduleCases.length !== 0
                    ) {
            ...
            (res) => {
                ...
                if (Number(res.code) === 200) {
                    if (res.body.length !== 0) {
                        commit("updateExpandedRowKeys", {
                            key: record.index,
                            expanded,
                        });
                        commit("updatecaseChildTableData", {
            ...
            (res) => {
                if (res.code === 200) {
                    dispatch(
                        ...
                        {
                            type: "success",
                            toast: "Successfully stoped testing",
                        },
                        { root: true }
                    );
                    data.status = "STOPPED";
    ...
    getEditTestJob({ dispatch, commit }, { data }) {
        axiosget(
            API.testJobMgt.testJobProgress.replace(":jobId", data.jobId)
        ).then(
            (res) => {
                if (res.code === 200) {
